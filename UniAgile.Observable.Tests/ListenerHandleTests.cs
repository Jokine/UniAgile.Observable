...
namespace UniAgile.Observable.Tests.ListenerHandleTests
{
    public class Unit
    {
        private static IListenerHandle[] CreateListenerHandles(Signal signal, Mock<Action>[] mockDelegates)
        => mockDelegates.Select(d => (IListenerHandle) new ListenerHandle(signal, d.Object))
                        .ToArray();
